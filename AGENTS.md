...
- TUI `/`-command parsing starts in `src/app/input.rs` and dispatches to `execute_command` in `src/app/commands_notes.rs`; the bottom-pane note editor lives in `src/app/notes_editor.rs` + `src/app/editor_input.rs`.
- CLI dispatch (`aleph <area> ...`) is `run_cli_command` in `src/app/core_accessors.rs`, which fans out to `run_room_cli_command` and `run_obsidian_cli_command` (in `src/app/core_accessors.rs`) and `run_trail_cli_command` and `run_daemon_cli_command` (in `src/app/trail.rs`).
...
- `cargo test` — run unit tests; the bulk live in `src/app/tests.rs`, with additional inline test modules in `src/app/{strix,rooms,obsidian,temporal_forks,trail,core_accessors}.rs` and `src/ui/tests.rs` (CI uses `cargo test --verbose`).
